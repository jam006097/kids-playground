...
- [x] **中タスク4.1: AIコアロジック `Summarizer` の実装**
    - [x] **小タスク4.1.1 (RED):** `Summarizer`クラスのテスト作成
        - **担当:** AI
    ...
    - [x] **小タスク4.1.2 (GREEN):** `Summarizer`クラスの骨格作成
        - **担当:** AI
        - **内容:** `src/ai_api/core/inference.py` と、空の`Summarizer`クラスを作成し、テストをパスさせます。
    - [x] **小タスク4.1.3 (REFACTOR):** リファクタリング
        - **担当:** AI
        - **内容:** この時点では特になし。コミットの区切りとします。

- [x] **中タスク4.2: AI設定クラス `Config` の実装と注入**
    - [x] **小タスク4.2.1 (RED):** `Summarizer`が設定オブジェクトを受け取るテスト作成
        - **担当:** AI
...
- [x] **中タスク4.3: 要約機能 `summarize` メソッドの実装**
    ...
    - [x] **小タスク4.3.2 (GREEN):** `summarize`メソッドの実装
        - **担当:** AI
        - **内容:** `Summarizer`クラスに`summarize`メソッドを実装し、内部で`transformers.pipeline`を呼び出して要約を実行するようにします。
    - [x] **小タスク4.3.3 (REFACTOR):** リファクタリング
        - **担当:** AI
        - **内容:** エラーハンドリングなどを追加し、コードを整理します。

- [x] **中タスク4.4: APIエントリーポイント `main.py` の実装**
    - [x] **小タスク4.4.1 (RED):** APIのインテグレーションテスト作成
        - **担当:** AI
