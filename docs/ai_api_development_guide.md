...
| **3. ユーザーフィードバック機構** | - 要約結果に「役に立ったか (👍/👎)」を投票できるUIを設計する。<br>- フィードバック結果をDjangoのDBに保存するためのデータモデルを設計する。<br>- 収集したデータを将来のモデル再学習やプロンプト改善にどう活かすか、方針を検討する。 | - UIデザイン案。<br>- `models.py`に追加するフィードバックモデルの定義。<br>- データ活用方針の概要。 |
...
| 項目 | 調査計画 | 成果物 |
| : | : | : |
| **1. 高度な入力フィルタリング** | - 悪意のある、または無関係な入力（スパム、不適切な単語など）を検知する手法を調査する。<br>- `profanity-filter`のようなライブラリや、キーワードブラックリスト方式の実現可能性を評価する。<br>- フィルタリング処理を推論パイプラインのどこに組み込むか設計する。 | - フィルタリング手法の比較と選定理由。<br>- 導入する場合の具体的な実装方針。 |
| **2. コスト・パフォーマンス試算** | - Hugging Face Spacesの有料プラン（ハードウェア性能）と料金体系を調査する。<br>- 1リクエストあたりの平均処理時間とメモリ使用量を計測する。<br>- 想定されるアクセス数に基づき、月間の運用コストと、必要となるハードウェアスペックを試算する。 | - コストとパフォーマンスの試算レポート。<br>- スケールアップが必要になる条件の定義。 |
...
| 項目 | 調査計画 | 成果物 |
| : | : | : |
| **1. ログ収集・分析基盤** | - Pythonの`logging`モジュールを使い、Gradio API側で構造化ログ（JSON形式）を出力する方法を調査する。<br>- Hugging Face Spacesのログ機能で、それらのログをどこまで閲覧・検索できるか確認する。<br>- ログに含めるべき情報（リクエストID, 処理時間, 入力文字数など）を定義する。 | - ログフォーマットの定義。<br>- Gradio/Djangoへのロギング設定例。 |
| **2. パフォーマンス監視** | - Hugging Face Spacesが標準で提供する分析機能（Analytics）で取得できるメトリクスを確認する。<br>- それだけでは不十分な場合、外部の監視ツール（Datadog, Sentryなど）と連携する方法を調査する。<br>- 特に監視すべき重要指標（KPI）を定義する（例: エラー率、95パーセンタイル応答時間）。 | - 監視ツールの選定と比較。<br>- 監視KPIリストとその閾値の定義。
...
- [ ] **中タスク3.2: Django Viewの実装**
    ...
    - [ ] **小タtスク3.2.2 (GREEN):** URLとViewの骨格作成
        - **担当:** AI
