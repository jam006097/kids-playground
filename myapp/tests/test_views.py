...
class PlaygroundTests(TestCase):
    ...
    def test_index_view(self):
        ...
        self.assertTemplateUsed(
            response, "index.html"
        )  # 使用されるテンプレートが正しいことを確認
        self.assertIn(
            "playgrounds", response.context
        )  # コンテキストにplaygroundsが含まれていることを確認
        self.assertIn(
            "total_count", response.context
        )  # コンテキストにtotal_countが含まれていることを確認
        self.assertIn(
            "filtered_count", response.context
        )  # コンテキストにfiltered_countが含まれていることを確認
        self.assertIn(
            "google_maps_api_key", response.context
        )  # コンテキストにgoogle_maps_api_keyが含まれていることを確認

    @patch("myapp.views.urllib.request.urlopen")
    def test_search_place_view(self, mock_urlopen):
        # search_placeビューのテスト（候補が見つかる場合）
        mock_response = {
            "candidates": [{"name": "Test Place", "formatted_address": "Test Address"}]
        }
        mock_urlopen.return_value.read.return_value = json.dumps(mock_response).encode()
        response = self.client.get(
            reverse("search_place"), {"name": "Test Place", "address": "Test Address"}
        )
        self.assertEqual(
            response.status_code, 200
        )  # ステータスコードが200であることを確認
